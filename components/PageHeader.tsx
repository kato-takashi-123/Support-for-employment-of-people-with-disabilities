...
export const PageHeader: React.FC<{ title: string; onBack?: () => void; onMenuClick?: () => void; }> = ({ title, onBack, onMenuClick }) => (
    <header className="bg-cyan-100 dark:bg-gray-800 shadow-sm sticky top-0 z-20 p-4 flex items-center justify-between h-12">
        ...
        <h1 className="text-sm xs:text-sm sm:text-base md:text-lg lg:text-xl font-bold text-gray-800 dark:text-gray-200 text-center absolute left-1/2 -translate-x-1/2 whitespace-nowrap max-w-[65%] overflow-hidden text-ellipsis">{title}</h1>
